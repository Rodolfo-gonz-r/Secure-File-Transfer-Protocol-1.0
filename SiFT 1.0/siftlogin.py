#python3

import time
from Crypto.PublicKey import RSA
from Crypto.Hash import SHA256
from Crypto.Protocol.KDF import HKDF, PBKDF2
from Crypto.Random import get_random_bytes
from siftmtp import SiFT_MTP, SiFT_MTP_Error
import secrets


class SiFT_LOGIN_Error(Exception):

    def __init__(self, err_msg):
        self.err_msg = err_msg

class SiFT_LOGIN:
    def __init__(self, mtp):

        self.DEBUG = True
        # --------- CONSTANTS ------------
        self.size_random = 16
        self.delimiter = '\n'
        self.coding = 'utf-8'
        self.accept_window = 2000000000 
        # --------- STATE ------------
        self.mtp = mtp
        self.server_pubkey = None
        self.server_keypair = None
        self.server_users = None 


    # sets the value of the server public key (to be used by the client)
    def set_server_pubkey(self, pubkey):
        self.server_pubkey = pubkey

    # sets the value of the server keypair (to be used by the server)
    def set_server_keypair(self, keypair):
        self.server_keypair = keypair

    # sets user passwords dictionary (to be used by the server)
    def set_server_users(self, users):
        self.server_users = users

    # builds a login request from a dictionary
    def build_login_req(self, login_req_struct):

        login_req_str = login_req_struct['timestamp']
        login_req_str += self.delimiter + login_req_struct['username'] 
        login_req_str += self.delimiter + login_req_struct['password'] 
        login_req_str += self.delimiter + login_req_struct['client_random'].hex()
        # login_req_str += self.delimiter + login_req_struct['client_random']
        
        
        return login_req_str.encode(self.coding)

    # parses a login request into a dictionary
    def parse_login_req(self, login_req):

        login_req_fields = login_req.decode(self.coding).split(self.delimiter)
        login_req_struct = {}
        login_req_struct['timestamp'] = int(login_req_fields[0])
        login_req_struct['username'] = login_req_fields[1]
        login_req_struct['password'] = login_req_fields[2]
        login_req_struct['client_random'] = bytes.fromhex(login_req_fields[3])
        return login_req_struct


    # builds a login response from a dictionary
    def build_login_res(self, login_res_struct):

        login_res_str = login_res_struct['request_hash'].hex() 
        login_res_str += self.delimiter + login_res_struct['server_random'].hex() 
        return login_res_str.encode(self.coding)


    
    # parses a login response into a dictionary
    def parse_login_res(self, login_res):

        login_res_fields = login_res.decode(self.coding).split(self.delimiter)
        login_res_struct = {}
        login_res_struct['request_hash'] = login_res_fields[0]
        login_res_struct['server_random'] = bytes.fromhex(login_res_fields[1])
        return login_res_struct
    

    # check correctness of a provided password
    def check_password(self, pwd, usr_struct):

        pwdhash = PBKDF2(pwd, usr_struct['salt'], len(usr_struct['pwdhash']), count=usr_struct['icount'], hmac_hash_module=SHA256)
        if pwdhash == usr_struct['pwdhash']: return True
        return False


    # handles login process (to be used by the server)
    def handle_login_server(self):

        if not self.server_keypair:
            raise SiFT_LOGIN_Error('Server keypair required for handling login at server')

        if not self.server_users:
            raise SiFT_LOGIN_Error('User database is required for handling login at server')

        # trying to receive a login request
        try:
            msg_type, msg_payload, tmp_key = self.mtp.receive_login_req(self.server_keypair)
        except SiFT_MTP_Error as e:
            raise SiFT_LOGIN_Error('Unable to receive login request --> ' + e.err_msg)

        # DEBUG 
        if self.DEBUG:
            print('Incoming payload (' + str(len(msg_payload)) + '):')
            print(msg_payload[:max(512, len(msg_payload))].decode('utf-8'))
            print('------------------------------------------')
        # DEBUG 

        if msg_type != self.mtp.type_login_req:
            raise SiFT_LOGIN_Error('Login request expected, but received something else')

        # processing login request
        hash_fn = SHA256.new()
        hash_fn.update(msg_payload)
        request_hash = hash_fn.digest()

        login_req_struct = self.parse_login_req(msg_payload)

        # checking timestamp
        current_time = time.time_ns()
        if (login_req_struct['timestamp'] > current_time + self.accept_window//2) or (login_req_struct['timestamp'] < current_time - self.accept_window//2):
            raise SiFT_LOGIN_Error('Freshness verification of login request failed')
        # CAUTION: 
        # We should also check that the same login request 
        # was not received in any other connection (client socket).
        # We can store request hashes for some time, and check
        # if the hash of this request is not among the stored ones.
        # We may implement this later...

        # checking username and password
        if login_req_struct['username'] in self.server_users:
            if not self.check_password(login_req_struct['password'], self.server_users[login_req_struct['username']]):
                raise SiFT_LOGIN_Error('Password verification failed')
        else:
            raise SiFT_LOGIN_Error('Unkown user attempted to log in')

        # building login response
        client_random = login_req_struct['client_random']
        server_random = get_random_bytes(self.size_random)

        login_res_struct = {}
        login_res_struct['request_hash'] = request_hash
        login_res_struct['server_random'] = server_random

        msg_payload = self.build_login_res(login_res_struct)

        # DEBUG 
        if self.DEBUG:
            print('Outgoing payload (' + str(len(msg_payload)) + '):')
            print(msg_payload[:max(512, len(msg_payload))].decode('utf-8'))
            print('------------------------------------------')
        # DEBUG 

        # setting MTP transfer key to temporary key
        self.mtp.set_transfer_key(tmp_key)

        # sending login response
        try:
            self.mtp.send_msg(self.mtp.type_login_res, msg_payload)
        except SiFT_MTP_Error as e:
            raise SiFT_LOGIN_Error('Unable to send login response --> ' + e.err_msg)

        # generate final transfer key from randoms
        transfer_key = HKDF(client_random + server_random, self.mtp.size_transfer_key, request_hash, SHA256)
        # DEBUG 
        if self.DEBUG:
            print("Client random: " + client_random.hex())
            print("Server random: " + server_random.hex())
            print("Request hash: " + request_hash.hex())        
            print("Final transfer key: " + transfer_key.hex())
        # DEBUG 

        # setting MTP transfer key to final transfer key
        self.mtp.set_transfer_key(transfer_key)

        # DEBUG 
        if self.DEBUG:
            print('User ' + login_req_struct['username'] + ' logged in')
        # DEBUG 

        return login_req_struct['username']


    
    # handles login process (to be used by the client)
    def handle_login_client(self, username, password):
        
        
        if not self.server_pubkey:
            raise SiFT_LOGIN_Error('Server public key required for handling login at client')
  
  
        # building a login request
        client_random = get_random_bytes(self.size_random)


        login_req_struct = {}
        login_req_struct['timestamp'] = str(time.time_ns())
        login_req_struct['username'] = username
        login_req_struct['password'] = password
        login_req_struct['client_random'] = client_random
        msg_payload = self.build_login_req(login_req_struct)

        # DEBUG 
        if self.DEBUG:
            print('Outgoing payload (' + str(len(msg_payload)) + '):')
            print(msg_payload[:max(512, len(msg_payload))].decode('utf-8'))
            print('------------------------------------------')
        # DEBUG

        # trying to send login request

        try:
            # self.mtp.send_login_req(msg_payload, self.server_pubkey)
            
            tmp_key = self.mtp.send_login_req(msg_payload, self.server_pubkey)
        except SiFT_MTP_Error as e:
            raise SiFT_LOGIN_Error('Unable to send login request --> ' + e.err_msg)

        # computing hash of sent request payload

        hash_fn = SHA256.new()
        hash_fn.update(msg_payload)
        request_hash = hash_fn.digest()

        # setting MTP 
        # to temporary key
        self.mtp.set_transfer_key(tmp_key)

        # trying to receive a login response
        try:
            msg_type, msg_payload = self.mtp.receive_msg()
        except SiFT_MTP_Error as e:
            raise SiFT_LOGIN_Error('Unable to receive login response --> ' + e.err_msg)

        # DEBUG 
        if self.DEBUG:
            print('Incoming payload (' + str(len(msg_payload)) + '):')
            print(msg_payload[:max(512, len(msg_payload))].decode('utf-8'))
            print('------------------------------------------')
        # DEBUG 

        if msg_type != self.mtp.type_login_res:
            raise SiFT_LOGIN_Error('Login response expected, but received something else')

        # parsing login response

        login_res_struct = self.parse_login_res(msg_payload)

        # checking request_hash receiveid in the login response
        # print("servers hash:")
        # print(login_res_struct['request_hash'])
        # print(type(login_res_struct['request_hash']))
        # print("clients hash")
        # # print(type(request_hash))
        # print(str(request_hash.hex()))
        # print(type(request_hash))
        if login_res_struct['request_hash'] != str(request_hash.hex()):
            raise SiFT_LOGIN_Error('Verification of login response failed')

        # # generate final transfer key from randoms
        
        server_random = login_res_struct['server_random']

        transfer_key = HKDF(client_random + server_random, self.mtp.size_transfer_key, request_hash, SHA256)

        # # setting MTP transfer key to final transfer key
        self.mtp.set_transfer_key(transfer_key)

